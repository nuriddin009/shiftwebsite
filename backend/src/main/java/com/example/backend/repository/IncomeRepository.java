...
public interface IncomeRepository extends JpaRepository<Income, UUID> {
    ...
    @Query("select sum(t.amount) from Income t where t.deleted is null or t.deleted is false ")
    Long getIncomeSum();

    @Query("select sum(t.amount)" +
            " from Income t where cast(t.created as date) = cast(:now as date) and (t.deleted is null or t.deleted is false)")
    ...
    @Query(
            " select sum(t.amount) from Income t where t.payType.id=:uuid and (t.deleted is null or t.deleted is false)"
    )
